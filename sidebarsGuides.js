/**
 * Sidebar for technical guides (guides/)
 */

// @ts-check

/** @type {import('@docusaurus/plugin-content-docs').SidebarsConfig} */
const sidebars = {
  guidesSidebar: [
    {
      type: 'category',
      label: 'Proxmox',
      link: {
        type: 'generated-index',
        description: 'Proxmox VE tutorials and troubleshooting',
      },
      items: [{type: 'autogenerated', dirName: 'proxmox'}],
    },
    {
      type: 'category',
      label: 'VMware',
      link: {
        type: 'generated-index',
        description: 'VMware ESXi administration and management',
      },
      items: [{type: 'autogenerated', dirName: 'vmware'}],
    },
    {
      type: 'category',
      label: 'XCP-ng',
      link: {
        type: 'generated-index',
        description: 'XCP-ng deployment, hardening, and VMware coexistence',
      },
      items: [{type: 'autogenerated', dirName: 'xcp-ng'}],
    },
    {
      type: 'category',
      label: 'Rocky Linux',
      link: {
        type: 'generated-index',
        description: 'Enterprise Linux configuration and best practices',
      },
      items: [{type: 'autogenerated', dirName: 'rocky-linux'}],
    },
    {
      type: 'category',
      label: 'Caddy',
      link: {
        type: 'generated-index',
        description: 'How to use, configure and deploy Caddy as a web server',
      },
      items: [{type: 'autogenerated', dirName: 'caddy'}],
    },
    {
      type: 'category',
      label: 'WireGuard',
      link: {
        type: 'generated-index',
        description: 'WireGuard VPN setup and configuration',
      },
      items: [{type: 'autogenerated', dirName: 'wireguard'}],
    },
    {
      type: 'category',
      label: 'Git',
      link: {
        type: 'generated-index',
        description: 'Git workflows and best practices',
      },
      items: [{type: 'autogenerated', dirName: 'git'}],
    },
    {
      type: 'category',
      label: 'Coolify',
      link: {
        type: 'generated-index',
        description: 'Coolify deployment platform guides and tutorials',
      },
      items: [{type: 'autogenerated', dirName: 'coolify'}],
    },
    {
      type: 'category',
      label: 'OpenCode',
      link: {
        type: 'generated-index',
        description: 'OpenCode setup, configuration, workflows, and troubleshooting',
      },
      items: [{type: 'autogenerated', dirName: 'opencode'}],
    },
    {
      type: 'category',
      label: 'ECC',
      link: {
        type: 'generated-index',
        description: 'Everything Claude Code — agent harness with 64 agents, 262 skills, 84 commands',
      },
      items: [{type: 'autogenerated', dirName: 'ecc'}],
    },
    {
      type: 'category',
      label: 'Caveman',
      link: {
        type: 'generated-index',
        description: 'Token compression — cut ~75% output tokens across 30+ AI agents',
      },
      items: [{type: 'autogenerated', dirName: 'caveman'}],
    },
  ],
};

export default sidebars;
